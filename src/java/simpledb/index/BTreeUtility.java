...
public class BTreeUtility {
    ...
    public static void dfs(BTreePage rootPage, TransactionId tid) throws Exception {
        if(rootPage instanceof BTreeLeafPage) {
            System.out.println(rootPage);
        } else {
            BTreeInternalPage p = (BTreeInternalPage) rootPage;
            ...
            while (iterator.hasNext()) {
                ...
                if(rightMost != null && !rightMost.equals(leftChild)) {
                    System.out.println("prev right child: " + rightMost + ", but cur left child" + leftChild);
                    throw new RuntimeException();
                }
                BTreePage page = (BTreePage) Database.getBufferPool().getPage(tid, leftChild, Permissions.READ_ONLY);
                dfs(page, tid);
                rightMost = entry.getRightChild();
            }
            dfs((BTreePage) Database.getBufferPool().getPage(tid, rightMost, Permissions.READ_ONLY), tid);
        }
    }

    public static void printAllPages(BTreeFile bf, TransactionId tid) throws Exception {
        BTreeRootPtrPage rootPtrPage = bf.getRootPtrPage(tid, new HashMap<>());
        BTreePageId rootId = rootPtrPage.getRootId();
        BufferPool bufferPool = Database.getBufferPool();
        BTreePage p = (BTreePage) bufferPool.getPage(tid, rootId, Permissions.READ_ONLY);
        dfs(p, tid);
    }
